...
import logging, os, re, sys, json, subprocess, time
...
timePat = "^[^0-9]*([0-9][0-9]:[0-9][0-9]:[0-9][0-9]).*$"

def timeToSec(datestr):
    for ds in datestr.split():
        m = re.match(".*([0-9][0-9]:[0-9][0-9]:[0-9][0-9]).*", ds)
        if m: return((array(m.group().split(':')).astype(int) * array([3600, 60, 1])).sum(axis=0))
